...
int cellular_op_manufacturer(struct cellular *modem, char *buf, size_t len)
{
    at_set_timeout(modem->at, 1);
    const char *response = at_command(modem->at, "AT+CGMI");
...
int cellular_op_model(struct cellular *modem, char *buf, size_t len)
{
    at_set_timeout(modem->at, 1);
    const char *response = at_command(modem->at, "AT+CGMM");
    strncpy(buf, response, len);
    buf[len-1] = '\0';
...
int cellular_op_revision(struct cellular *modem, char *buf, size_t len)
{
    at_set_timeout(modem->at, 1);
