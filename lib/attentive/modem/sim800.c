...
/* No POSIX support means that this header needs to go away !
#include <unistd.h> */

#include "../../gps_nmea_parser/include/gps/gps.h"
#include "common.h"

/* Global GPS Handle / Available. */
static gps_t hgps;
ATOMIC_DEFINE(gps_data_available, 1);
...
#define SIM800_SOCKET_STATUS_ERROR     -1
#define SIM800_SOCKET_STATUS_UNKNOWN    0
#define SIM800_SOCKET_STATUS_CONNECTED  1
...
struct cellular_sim800 {
    ...
    int ftpget1_status;
    int socket_status[SIM800_NSOCKETS];
};
...
static int sim800_config(struct cellular *modem, const char *option, const char *value, int attempts)
{
    ...
    for (int i=0; i<attempts; i++) {
        ...
        /* BWASIM: Converted sleep(1) to k_sleep(K_SECONDS(1)) to remove POSIX dependency. */
        k_sleep(K_SECONDS(1));
    }
...
#endif /* Support for NTP is not required at this point. */
...
/*    printk("Socket Connect sees connection id: %d\r\n", connid); */
    ...
    /*
    	printk("Here with priv->socket_status[connid]: %d \r\n", priv->socket_status[connid]);
    	printk("Address: %d \r\n", &priv->socket_status[connid]);
    	printk("[connid]: %d \r\n", connid);
    */
...
static int sim800_socket_send(struct cellular *modem, int connid, const void *buffer, size_t amount, int flags)
{
    (void) flags;
...
static int sim800_socket_recv(struct cellular *modem, int connid, void *buffer, size_t length, int flags)
{
    (void) flags;
...
/* Function: SIM80x_Get_GPS
 * Description: This function asks the SIM80x Module to get GPS from SIM Module. */
void SIM80x_Get_GPS(gps_t *gps) {

	/* Get device. */
	struct cellular *modem = simcomm_sim80x_dev_get();

	/* Attach the GPS. */
	if (at_command(modem->at, "AT+CGNSPWR=1") != NULL) {
		if (at_command(modem->at, "AT+CGNSTST=1") != NULL)  {

			/* Wait until the URC handler gives us the go ahead to read data. */
			while (atomic_get(gps_data_available) == false);

			/* Lets read the GPS co-ordinates. */
			memcpy(gps, (void *) &hgps, sizeof(gps_t));

			/* De-attach the GPS. */
			(void) at_command(modem->at, "AT+CGNSPWR=0");
...
static const struct cellular_ops sim800_ops = {
	...
	.manufacturer = cellular_op_manufacturer,
	.model        = cellular_op_model,
	.revision     = cellular_op_revision,
...
/*
    struct cellular_sim800 *modem = &sim800_cellular_dev;
  */
    if (modem == NULL) {
        errno = ENOMEM;
        return NULL;
...
{
	/* No implementation of free available.
	 * free(modem); */
}
